...
var indexLePlusLong = 0;

for (var i = 1; i < phrases.length; i++) {
    if (phrases[indexLePlusLong].length < phrases[i].length) {
        indexLePlusLong = i; // ecrase la valeur precedente
        
    }
}

document.write('la phrase la plus longue est : '+ phrases[indexLePlusLong]);
document.write(' caracteres : '+ phrases[indexLePlusLong].length);
